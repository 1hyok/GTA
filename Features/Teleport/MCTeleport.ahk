; MC 텔레포트 실행 함수
ExecuteMCTeleport(key) {
    global mcRunning, keyPressed, config
    ...
    ; 단계 3: 보스 등록
    ExecuteBossRegistrationSequence(phoneCloseDelay, menuOpenDelay, menuControlDelay, key)
...
ExecuteBossRegistrationSequence(phoneCloseDelay, menuOpenDelay, menuControlDelay, key) {
    ...
    if (key = config["Hotkeys"]["TeleportMC1"]) {
        ShowMCMessage("🏢 CEO 등록 준비...", 500)
        PressKey("Down", 1, menuControlDelay)
        Sleep(menuControlDelay)
    } else if (key = config["Hotkeys"]["TeleportMC2"]) {
